...
L.equipmenteditor_desc_not_buyable = "無効にしたら、ショップに表示されないようになる。\nこのアイテムが割り当てられている役職は引き続きそれを受け取ります。"
L.equipmenteditor_name_not_random = "常時購入可能"
L.equipmenteditor_desc_not_random = "有効にしたら、常時このアイテムがショップに表示されるようになり、ランダムショップの影響は受けません。\n１つの使用可能なランダムスロットを取り、常にこのアイテムのためにそれが予約されます。"
L.equipmenteditor_name_global_limited = "全体的な使用制限"
L.equipmenteditor_desc_global_limited = "有効にしたら、一つ買ったら次のラウンドまで他の者たちは買うことはできません。"
L.equipmenteditor_name_team_limited = "陣営による使用制限"
L.equipmenteditor_desc_team_limited = "有効にしたら、一つ買ったら次のラウンドまで他のチームメイトは買うことはできません。"
L.equipmenteditor_name_player_limited = "使用制限"
L.equipmenteditor_desc_player_limited = "有効にしたら、一つ買ったら次のラウンドまで買うことはできません。"
...
デフォルトでは、その武器の製作者が「AutoSpawnable」とマークした武器だけマップに生成されますが、これらの設定はこのメニューから変更できます。
ほとんどの装備は、デフォルトで「特殊武器」に設定されています。つまり、装備がランダム武器のスポーン位置にのみ生成されるということです。
しかし、マップ上に特殊な武器のスポーンを配置したり、他の既存のスポーンタイプを使用するためにここでスポーンタイプを変更することは可能です。]]
...
前述の役職配布は、プレイヤーの下限との特別な統合を持っています。\n役職が選択対象と見なされ、最小値は分布係数によって指定された値を下回っていますが、プレイヤー人数が下限以上の場合、\n1人のプレイヤーがこの役職を引き続き受けることができます。この役職を受け取る2番目のプレイヤーの配布設定は、再度trueになります。]]
...
基本役職は、他のユーザーが継承する役職のみです。例えば、Innocentは基本役職ですが、Pharaohはこの役職のサブ役職です。デフォルトでは、割り当て可能な異なる基本役職の数に制限はありません。ただし、それらを制限する2つの異なる方法があります。
...
チーム全体が授与された場合でも、この機能は、すべての役職で有効/無効にすることができます。例えば、Innocent陣営に授与される時、Innocentがこの設定で無効になっている場合、Detectiveだけがクレジットを受け取ります。
...
L.help_item_armor_classic = "クラシックアーマーモードは、プレイヤーがラウンドで一度だけボディアーマーを購入することができ、アーマーは弾丸とバールによるダメージの30%を軽減できます。"
...
L.label_crowbar_unlocks = "プライマリーアタックは相互作用としても使用されます。つまり、鍵を解除できます。"
