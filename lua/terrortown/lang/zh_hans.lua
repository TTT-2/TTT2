...
L.spawn_weapon_edit_ammo = "按住 [{walkkey}]并按 [{primaryfire} 或 {secondaryfire}] 以增加或减少此武器生成的弹药"

L.spawn_type_weapon = "这是一个武器生成"
L.spawn_type_ammo = "这是弹药生成"
L.spawn_type_player = "这是一个玩家生成"

L.spawn_remove = "按 [{secondaryfire}] 删除此生成"

L.submenu_administration_entspawn_title = "再生编辑"
L.header_entspawn_settings = "再生编辑器设置"
L.button_start_entspawn_edit = "开始再生编辑"
L.button_delete_all_spawns = "删除所有生成位置"
...
生成编辑器是用来放置，移除和编辑世界上的生成的。这些生成点是为武器，弹药和玩家准备的。

这些生成点被保存在位于"data/ttt/weaponspawnscripts/"的文件中。它们可以被删除以进行硬重置。最初的生成文件是由地图上和原始TTT武器生成脚本中的生成创建的。按下重置按钮总是会恢复到这个状态。

应该注意的是，这个生成系统使用动态生成。这对武器来说是最有趣的，因为它不再定义一个特定的武器，而是定义一种武器类型。例如，现在不是TTT霰弹枪的生成，而是一般霰弹枪的生成，任何定义为霰弹枪的武器都可以生成。每个武器的生成类型可以在装备编辑器中设置。这使得任何武器都可以在地图上生成，或者禁用某些默认武器。

请记住，许多变化只有在新的一轮开始后才会生效]]
...
TTT2的生成系统允许每种武器在世界中生成。默认情况下，只有被创造者标记为'自动生成'的武器才会在世界中生成，但是这些设置可以在这个菜单中改变。

大多数装备在默认情况下被设置为'特殊武器生成'。这意味着装备只在随机武器生成点上生成。然而，我们可以在世界中放置特殊的武器生成点，或者改变这里的生成类型，以使用其他现有的生成类型。。]]

L.pickup_error_inv_cached = "你现在不能拿起这个，因为你的库存被缓存了。"

-- 2021-09-02
L.submenu_administration_playermodels_title = "玩家模型"
L.header_playermodels_general = "一般玩家模型设置"
L.header_playermodels_selection = "选择玩家模型库"

L.label_enforce_playermodel = "强制执行玩家模型"
...
有些地图定义了他们自己的玩家模型。默认情况下。这些模型的优先级比自动分配的模型高。通过禁用此设置。地图--特定的模型被禁用。

角色特定的模型总是有更高的优先权。不受这个设置的影响。]]
L.help_enforce_playermodel = [[
有些角色有自定义的球员模型。这可以被禁用，这可能与一些播放器模型选择器的兼容性有关。

如果这个设置被禁用，仍然可以选择随机默认模型。]]
L.help_use_custom_models = [[
默认情况下，只有CSS Phoenix的球员模型被分配给所有球员。然而，通过启用这个选项，可以选择一个球员模型库。启用此设置后，每个玩家仍将被分配到相同的玩家模型，但它是定义的模型池中的一个随机模型。
...
L.tooltip_kill_score_team = "团队击杀: {score}"
L.kill_score_team = "团队击杀:"

-- 2021-10-09
L.help_models_select = [[
左键点击模型，将其添加到玩家模型库中。再次左键单击以删除它们。右键单击可在所关注的模型的启用和禁用侦探帽之间进行切换。
	
左上角的小指示器显示玩家模型是否有一个爆头的命中箱。下面的图标显示了这种模式是否适用于侦探帽。]]

L.menu_roles_title = "角色设置"
L.menu_roles_description = "设置生成，设备学分和更多内容。"

L.submenu_administration_roles_general_title = "一般角色设置"

L.header_roles_info = "角色信息"
L.header_roles_selection = "角色选择参数"
L.header_roles_tbuttons = "角色叛徒按钮"
L.header_roles_credits = "角色装备积分"
L.header_roles_additional = "额外的角色设置"
L.header_roles_reward_credits = "奖励设备学分"

L.help_roles_default_team = "默认团队: {team}"
L.help_roles_unselectable = "这个角色是不可选择的。这意味着它在角色选择中不被考虑。大多数情况下，这意味着这是一个在回合中通过一个事件(如复兴，副手老鹰或类似的东西)手动应用的角色。。"
L.help_roles_selectable = "这个角色是可选择的。这意味着如果满足所有的标准，这个角色在角色选择过程中会被考虑。"
L.help_roles_credits = "装备点数用于在商店购买装备。大多数情况下，只给那些可以进入商店的角色信用额度是有意义的。然而，由于有可能从尸体上掠夺学分，也可以考虑给角色提供起始学分，作为对他们杀手的奖励。"
L.help_roles_selection_short = "每个玩家的角色分布定义了被分配到这个角色的玩家的百分比。例如，如果该值被设置为'0。2'，那么每五个玩家就会收到这个角色。"
L.help_roles_selection = [[
每个玩家的角色分配定义了被分配到这个角色的玩家的百分比。例如，如果该值被设置为 "0。2"，那么每五名球员就会得到这个角色。这也意味着，至少需要5名球员才能选择这个角色。
请记住，所有这些都只适用于该角色被考虑选择的情况。
...
后者仅在固定金额为'0'时使用，并根据设定的可用玩家百分比设置上限。]]
L.help_roles_max_baseroles = [[
基层角色只是那些其他角色所继承的角色。例如，"无辜者"角色是一个基本角色，而"法老"是这个角色的一个子角色。默认情况下，对于可以分配多少个不同的角色没有限制。然而，这里有两种不同的方法来限制它们。

1。用一个固定的数量来限制它们。
2。按百分比限制。

后者只在固定金额为'0'时使用，并根据设定的可用玩家百分比设置上限。]]

L.label_roles_enabled = "启用角色"
L.label_roles_min_inno_pct = "将成为平民的玩家总数百分比"
L.label_roles_pct = "每个游戏者的角色分配"
L.label_roles_max = "分配给这个角色的玩家上限"
L.label_roles_random = "分配这个角色的可能性"
L.label_roles_min_players = "考虑分配这个角色的下限"
L.label_roles_tbutton = "是否启用叛徒一队"
L.label_roles_credits_starting = "初始设备学分"
L.label_roles_credits_award_pct = "学分奖励报酬率"
L.label_roles_credits_award_size = "学分奖励报酬规模"
L.label_roles_credits_award_repeat = "重复学分奖励报酬"
L.label_roles_newroles_enabled = "启用自定义角色"
L.label_roles_max_roles = "角色上限"
L.label_roles_max_roles_pct = "按百分比计算的角色上限"
L.label_roles_max_baseroles = "基底层上限"
L.label_roles_max_baseroles_pct = "按百分比计算的基底上限"
L.label_detective_hats = "为像侦探这样的警察角色启用帽子(如果玩家模型允许戴帽子)。"

L.ttt2_desc_innocent = "无辜者没有特殊能力。他们必须在恐怖分子中找到邪恶的人并杀死他们。但他们必须小心，不要杀死他们的同伴。"
L.ttt2_desc_traitor = "叛徒是无辜者的对手。他们有一个装备菜单，可以购买特殊装备。他们必须杀死所有的人，除了他们的队友。"
L.ttt2_desc_detective = "侦探是无辜者可以信任的人。但谁是无辜者？强大的侦探必须要找到所有邪恶的恐怖分子。他们商店里的设备可能会帮助他们完成这项任务。"

-- 2021-10-10
L.button_reset_models = "重置玩家模型"

-- 2021-10-13
L.help_roles_credits_award_kill = "另一种获得学分的方式是通过杀死具有'公共角色'(如侦探)的高价值玩家。如果杀手的角色启用了这个功能，他们就会获得以下规定的信用额度。"
L.help_roles_credits_award = [[
在TTT2基地，有两种不同的方式可以获得学分:

1。如果敌方队伍中有一定比例的人死亡，整个队伍就会被授予学分。
2。如果一个玩家用'公共角色'(如侦探)杀死了一个高价值的角色，那么这个杀手就会得到奖励。

请注意，即使全队都获得奖励，这仍然可以为每个角色启用/禁用。例如，如果团队中的无罪者被奖励，但无罪者的角色被禁用，只有侦探会收到他们的积分。
这个功能的平衡值可以在'管理'->'一般角色设置'中设置。]]
L.help_detective_hats = [[
侦探等警察角色可以戴帽子以显示其权威。他们在死亡时或头部受损时将失去帽子。

一些玩家模型默认不支持帽子。你可以在'管理'->'玩家模型'中改变这一点。']]
...
L.label_roles_min_karma = "考虑选择的人品值下限"
...
L.header_administration_general = "一般管理设置"
L.header_administration_logging = "日志"
L.header_administration_misc = "杂项"
L.header_entspawn_plyspawn = "玩家再生设置"
L.header_voicechat_general = "一般语音聊天设置"
L.header_voicechat_battery = "Voicechat电池"
L.header_voicechat_locational = "定位语音聊天"
L.header_playersettings_plyspawn = "玩家再生设置"
L.header_round_setup_prep = "回合准备"
L.header_round_setup_round = "初始回合"
L.header_round_setup_post = "回合结束"
L.header_round_setup_map_duration = "地图会话"
L.header_textchat = "文本聊天"
L.header_round_dead_players = "死亡玩家设置"
L.header_administration_scoreboard = "记分牌设置"
...
L.header_playersettings_armor = "盔甲系统设置"

L.header_killer_dna_range = "当玩家被其他玩家杀死时，会在他们身上留下DNA指纹。最大范围convar定义了留下DNA样本的最大距离，以锤子为单位。如果杀手在更远的地方，那么就不会在尸体上留下样本。"
L.help_killer_dna_basetime = "直到DNA样本衰变的基本时间，以秒为单位。从这个基准时间中减去一个杀手距离的平方系数。"
L.help_dna_radar = "如果配备了TTT2 DNA扫描器，会显示所选DNA样本的确切距离和方向。然而，也有一种经典的DNA扫描器模式，每次冷却时间过后都会用世界范围内的渲染来更新所选的样本。"
L.help_idle = "空闲模式是用来将空闲的玩家转移到一个强制的旁观者模式。要再次离开这个模式，他们必须在他们的'游戏'设置中禁用'强制旁观模式'。"

L.help_namechange_kick = [[
如果玩家在一个回合中改变他们的名字，这可能会被滥用来逃避被杀。因此，禁止在一个活动回合中改变昵称。

如果bantime大于0，该玩家将无法重新连接到服务器，直到该时间过去。]]
L.help_damage_log = "每次玩家受到伤害时，如果启用的话，就会在控制台中添加一个伤害日志条目。这也可以在一个回合结束后存储到磁盘上。该文件位于'data/terrortown/logs/'"
L.help_spawn_waves = [[
如果这个变量被设置为0，所有的玩家都是一次性生成的。对于拥有大量玩家的服务器来说，一波一波地催生玩家可能是有益的。生成波的时间间隔是指每个生成波之间的时间。一个生成波总是产生与有效生成点相同数量的玩家。

注意:确保准备时间足够长，以达到所需的生成波数量。]]
L.help_voicechat_battery = [[
在启用语音聊天电池的情况下，语音聊天会减少这个仪表。当它是空的时候，玩家就不能语音聊天，必须等待几秒钟来充电。这可以帮助防止过度使用语音聊天。

注意:'Tick'指的是游戏中的Tick，即1/66秒的时间。]]
L.help_ply_spawn = "在玩家(重新)生成时使用的玩家参数。"
L.help_haste_mode = [[
急速模式通过增加每一个死亡玩家的回合时间来平衡游戏。只有看到行动中失踪的玩家的角色才能看到真正的回合时间。其他角色只能看到急速模式的起始时间。

如果急速模式被启用，固定的回合时间将被忽略。]]
L.help_round_limit = "在满足设定的限制条件之一后，会触发地图变化。"
L.help_armor_balancing = "以下数值可以用来平衡盔甲。"
L.help_item_armor_classic = "如果启用了经典盔甲模式，只有之前的设置才是重要的。经典盔甲模式意味着玩家在一个回合中只能购买一次盔甲，并且这个盔甲可以阻挡30%的子弹和铁棍的来袭伤害，直到他们死亡。"
L.help_item_armor_dynamic = [[
动态装甲是TTT2的方法，使装甲更加有趣。现在可以购买的盔甲数量是无限的，而且盔甲的价值可以叠加。受到伤害会降低护甲值。每件捕获的盔甲物品的护甲值是在该物品的 "装备设置 "中设置的。

当受到伤害时，这个伤害的一定比例会转化为盔甲伤害，不同的比例仍然适用于玩家，其余的则会消失。
...
L.help_karma = "人品值是用来减少随机杀戮的。玩家开始时有一定数量的人品值，当他们伤害/杀死队友时就会失去人品值。他们失去的数量取决于他们伤害或杀死的人的人品值。较低的人品值会减少给予的伤害。"
L.help_karma_strict = "如果严格的因果报应被启用，伤害惩罚会随着因果报应的减少而更快增加。当它关闭时，当人们保持在800以上时，伤害惩罚是非常低的。启用严格模式使人品值在阻止任何不必要的杀戮方面发挥更大的作用，而禁用它则导致一个更'宽松'的游戏，人品值只伤害那些不断进行团队杀戮的玩家。"
L.help_karma_max = "将最大人品值设置为1000以上，不会给人品值超过1000的玩家提供伤害奖励。它可以作为一个人品值缓冲区。"
L.help_karma_ratio = "用于计算如果双方在同一个团队中，受害者的人品值被减去多少的伤害比例。如果发生团队杀戮，会有进一步的惩罚。"
L.help_karma_traitordmg_ratio = "如果双方在不同的队伍中，用来计算受害者的人品值被减去多少的伤害比率。如果发生团队杀戮，会有进一步的奖励。"
L.help_karma_bonus = "在一个回合中也有两种不同的被动方式来获得人品值。首先，一个回合的治疗会应用于每个玩家。然后，如果没有队友受伤或被杀，会有一个二次清洁的奖励。"
L.help_karma_clean_half = [[
当玩家的人品值高于起始水平时(意味着人品值最大值已被配置为高于该水平)，他们所有的人品值增加将根据其人品值高于起始水平的程度而减少。因此，它越高，增加的速度就越慢。

这种减少是以指数衰减的曲线进行的:最初速度很快，随着增量的变小，速度也会变慢。这个 convar 设定了奖金减半的时间点(即半衰期)。在默认值为0。25的情况下，如果人品值的起始值为1000，最大金额为1500，而玩家有人品值1125((1500-1000)*0。25=125)，那么他的清洁轮奖励将是30/2=15。因此，为了使奖金下降得更快，你应该把这个信念设置得更低，为了使它下降得更慢，你应该把它增加到1。]]
...
L.label_killer_dna_range = "留下DNA的最大杀伤范围"
L.label_killer_dna_basetime = "样本生命基础时间"
...
L.label_crowbar_shove_delay = "撬棍推后的冷却时间"
L.label_idle = "启用空闲模式"
...
L.label_ragdoll_pinning = "为非无辜的角色启用布娃娃夹子"
L.label_ragdoll_pinning_innocents = "为无辜的角色启用布娃娃夹击"
...
L.label_voice_drain_admin = "管理员和公共警察角色的每滴答都会耗尽"
L.label_voice_drain_recharge = "不进行语音聊天时每格的充值率"
L.label_locational_voice = "为活着的玩家启用3D语音聊天声音"
L.label_armor_on_spawn = "玩家在(重新)出生时的护甲"
L.label_prep_respawn = "在准备阶段启用即时重生"
L.label_preptime_seconds = "准备时间(秒)"
L.label_firstpreptime_seconds = "第一次准备时间(秒)"
L.label_roundtime_minutes = "固定回合时间(分钟)"
L.label_haste = "启用急速模式"
L.label_haste_starting_minutes = "急速模式开始时间(分钟)"
L.label_haste_minutes_per_death = "每次死亡的速度奖励(分钟)"
...
L.label_confirm_killlist = "宣布确认尸体的杀戮名单"
L.label_inspect_detective_only = "限制对警察角色的尸体检查"
L.label_confirm_detective_only = "只对警察角色进行尸体确认"
L.label_dyingshot = "如果在铁丝网内，则在死亡时开枪[试验性]"
L.label_armor_block_headshots = "启用盔甲阻挡爆头"
L.label_armor_block_blastdmg = "启用盔甲阻挡爆炸伤害"
L.label_armor_dynamic = "启用动态装甲"
L.label_armor_value = "护甲物品所赋予的护甲"
L.label_armor_damage_block_pct = "盔甲承受的伤害百分比"
L.label_armor_damage_health_pct = "玩家承受的伤害百分比"
L.label_armor_enable_reinforced = "启用强化装甲"
L.label_armor_threshold_for_reinforced = "强化装甲阈值"
L.label_sherlock_mode = "启用神探模式"
L.label_highlight_admins = "突出服务器管理员"
L.label_highlight_dev = "突出显示TTT2开发者"
L.label_highlight_vip = "突出显示VIP"
L.label_highlight_addondev = "突出显示TTT2附加组件的开发者"
L.label_highlight_supporter = "突出显示其他支持者"
L.label_enable_hud_element = "启用{elem} HUD元素"
L.label_spec_prop_control = "启用道具占有"
L.label_spec_prop_base = "持有的基本价值"
L.label_spec_prop_maxpenalty = "占有奖金下限"
L.label_spec_prop_maxbonus = "占有率奖金上限"
L.label_spec_prop_force = "占有的推动力"
L.label_spec_prop_rechargetime = "充能时间(秒)"
L.label_doors_force_pairs = "强迫靠近的门成为双门"
L.label_doors_destructible = "启用可毁坏的门"
L.label_doors_locked_indestructible = "初始锁定的门是不可摧毁的"
L.label_doors_health = "门的健康状况"
L.label_doors_prop_health = "门道具健康状况"
L.label_minimum_players = "开始游戏的最低玩家数量"
L.label_karma = "启用人品值"
L.label_karma_strict = "启用严格的人品值"
L.label_karma_starting = "开始人品值"
...
L.label_karma_persist = "人品值在地图变化中持续存在"
...
L.label_max_primary_slots = "最大的主要插槽"
L.label_max_nade_slots = "最大nade插槽"
L.label_max_carry_slots = "最大携带槽位"
L.label_max_unarmed_slots = "最大的非武装槽位"
L.label_max_special_slots = "最大的特殊槽位"
L.label_max_extra_slots = "最大的额外槽位"
L.label_weapon_autopickup = "启用自动武器拾取"
L.label_sprint_enabled = "启用冲刺功能"
L.label_sprint_max = "最大冲刺体力"
L.label_sprint_stamina_consumption = "体力消耗系数"
L.label_sprint_stamina_regeneration = "体力再生系数"
L.label_sprint_crosshair = "冲刺时显示十字准线"
L.label_crowbar_unlocks = "主要攻击可以作为互动(即解锁)使用"
