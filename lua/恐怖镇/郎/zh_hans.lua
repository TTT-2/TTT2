...
-- 如果你不知道该如何正确翻译或完全使用了机器翻译，请不要提交分支拉取请求！/ Don't use google translations!
...
L.c4_disarm_t     = "剪断引线以拆除C4。由于你是叛徒，所以每根电线都是安全的。无辜者可就没那么容易了！"
...
L.itemeditor_name_not_buyable = "可购买的装备"
L.itemeditor_desc_not_buyable = "如果禁用，该装备将不会显示在商店里。分配了这种装备的角色仍然会获得它。"
L.itemeditor_name_not_random = "永远可用"
L.itemeditor_desc_not_random = "如果启用，该装备会在商店里总是可用的。这在使用随机商店时会占用一个装备槽，并总是为这个装备保留。"
L.itemeditor_name_global_limited = "全局限量"
L.itemeditor_desc_global_limited = "如果装备是全局限量的，那么它只能在单次回合中购买一次。"
L.itemeditor_name_team_limited = "阵营限量"
L.itemeditor_desc_team_limited = "如果装备是阵营限量的，那么它在单次回合中每阵营只能买一次。"
L.itemeditor_name_player_limited = "玩家限量"
L.itemeditor_desc_player_limited = "如果装备是玩家限量的，那么它在单次回合中每人只能买一次。"
L.itemeditor_name_min_players = "有多少玩家时可选择"
L.itemeditor_name_credits = "价格以积分计算"
...
L.spawneditor_ammo_edit = "按住以编辑武器生成时的默认备弹量"
...
目前在这张地图上有{weapon}武器生成，{ammo}弹药生成和{player}玩家生成。点击'开始编辑生成'来改变这个生成。
...
L.menu_server_addons_description = "服务器内插件，仅管理员可以设置。"
...
左键点击模型，将其添加到玩家模型库中。再次左键以删除它们.，右键可在所关注的模型的启用和禁用侦探帽之间进行切换。
...
每个玩家的角色分配定义了被分配到这个角色的玩家的百分比。例如，如果该值被设置为 "0.2"，那么每五名玩家中就有一人会变为此角色。这也意味着，至少需要5名玩家才会启用此角色。
请记住，这些理论只适用于该角色为可被选择的情况。

前面提到的角色分配与玩家的下限有一个特殊的组合。如果角色可被选择，并且最小值低于分配系数给定的值，但玩家数量等于或大于下限，单个玩家仍然可以变为该角色。随后，对于第二个变为这个角色的玩家来说，分配设置将再次成立。]]
L.help_roles_award_info = "部分角色（如果在他们的积分设置中启用）在一定比例的对手死亡后会获得装备积分，该数值可在这里进行调整。"
L.help_roles_award_pct = "当其他玩家死亡人数超过该百分比后，玩家会获得更多的积分。"
L.help_roles_award_repeat = "积分奖励是否会多次发放.例如，如果你将百分比设置为'0.25'，并启用此功能，玩家将在死亡人数到达全玩家的'25%'、'50%'、'75%'时获得积分。"
L.help_roles_advanced_warning = "警告：这些是高级设置，将可能完全扰乱你的角色选择。如果有疑问，请将所有值保持在'0'。这个值意味着不应用任何限制，角色选择系统将试图分配尽可能多的角色。"
...
L.label_roles_credits_award_repeat = "重复学积分奖励"
...
L.ttt2_desc_innocent = "无辜者没有特殊能力，他们必须在恐怖分子中找到图谋不轨的人并杀死他们。但他们得小心，不要误杀自己的同伴。"
L.ttt2_desc_traitor = "叛徒是无辜者中的内鬼。他们拥有装备菜单，可以购买特殊装备。叛徒们必须杀死除自己队友之外的所有人。"
...
-- 2021-10-13
L.help_roles_credits_award_kill = "另一种获得积分的方式是通过杀死为'公开角色'(如侦探)的高价值玩家。如果非无辜者阵营的角色启用了这个功能，他们就会获得以下规定的积分。"
...
L.header_killer_dna_range = "当玩家被其他玩家杀死时，会在他们身上留下DNA指纹，最大范围convar定义了留下DNA样本的最大距离，以锤子编辑器为单位，如果杀手在更远的地方，那么就不会在尸体上留下样本。"
...
L.label_confirm_detective_only = "只让警察角色进行尸体确认"
...
L.label_doors_prop_health = "Prop门健康值"
...
L.label_max_nade_slots = "Nade槽位最多可携带"
