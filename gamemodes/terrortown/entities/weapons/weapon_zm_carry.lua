...
function SWEP:CheckValidity()
	local ctarget = self:GetCarryTarget()
	if (ctarget and not IsValid(ctarget)) or (self.Constr and not IsValid(self.Constr)) or (self.CarryHack and not IsValid(self.CarryHack)) then
		self:Reset()
...
function SWEP:MoveObject(phys, pdir, maxforce, is_ragdoll)
	...
	local force = math.Remap(speed, self.moveObjectRemapSpeedInMin, self.moveObjectRemapSpeedInMax, maxforce, self.moveObjectRemapSpeedOutMin)
	...
	if mass < self.moveObjectLightObjectMass then
		pdir = pdir * (mass + self.moveObjectLightObjectAdditionalMass) * self.moveObjectLightObjectMassScale
	end
...
function SWEP:CanBeMoved(target)
	...
	if not IsValid(phys)
		...
		or phys:HasGameFlag(FVPHYSICS_PLAYER_HELD)
	then
		 return false
	end
...
function SWEP:DoAttack(pickup)
	...
	if pickup then
		if self:AllowPickup(trEnt) then
			...
			local delay = (ctype == CARRY_TYPE_RAGDOLL) and self.carryPickupRagdoll or self.carryPickupDelay
...
function SWEP:Pickup()
	...
	if CLIENT or IsValid(ctarget) then return end
...
function SWEP:PinRagdoll()
	if CLIENT or not self:CanRagPin() then return end
	...
	if tr.HitWorld and not tr.HitSky then
		...
		constraint.Rope(ctarget, tr.Entity, bone, 0, bonepos, attachpos, length, length * self.pinBoneLengthFactor, self.moveForce, self.pinRopeWidth, self.pinMaterial, false, color_white)
...
if SERVER then
	...
	function SWEP:Think()
		...
		if ctype == CARRY_TYPE_NONE or not self:CheckValidity() then return end
...
if CLIENT then
	...
	function SWEP:RefreshTTT2HUDHelp()
		...
		if ctype ~= CARRY_TYPE_NONE and IsValid(ctarget) then
			if ctype == CARRY_TYPE_RAGDOLL then
				self:AddTTT2HUDHelp(
					self:CanPinCurrentRag() and "magneto_stick_help_carry_rag_pin" or "magneto_stick_help_carry_rag_drop",
					"magneto_stick_help_carry_rag_drop"
				)
			elseif ctype == CARRY_TYPE_PROP or ctype == CARRY_TYPE_WEAPON then
				self:AddTTT2HUDHelp(
					cvPropThrow:GetBool() and "magneto_stick_help_carry_prop_release" or "magneto_stick_help_carry_prop_drop",
	...
	function SWEP:CanPinCurrentRag()
		...
		local ctype = DetermineCarryType(ctarget)
		local ply = self:GetOwner()
		if ctype == CARRY_TYPE_RAGDOLL and IsValid(ctarget) and self:CanRagPin() and ply:IsTerror() then
			local tr = util.TraceLine({
				start = ply:EyePos(),
				endpos = ply:EyePos() + ply:GetAimVector() * self.pinRagRange,
				filter = {
