...
function ENT:Initialize()
	...
	if SERVER then
		self.trail = util.SpriteTrail( self, 0, self.trail_color, true, self.trail_width, self.trail_end_width, self.trail_lifetime, self.trail_lifetime, self.trail_texture )
		self:DeleteOnRemove(self.trail)
	end

	self.real_scale = self:GetFlameSize() * (self.fire_scale_base + math.Rand(-self.fire_scale_variance, self.fire_scale_variance))
