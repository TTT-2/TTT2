...
local function DrawClose(w, h, backgroundColor, textColor)
	local padding = 15

	draw.Box(0, 0, w, h, backgroundColor)
	draw.FilteredShadowedTexture(padding, padding, w - 2 * padding, h - 2 * padding, materialClose, textColor.a, textColor)
end
...
function SKIN:PaintWindowCloseButton(panel, w, h)
	...
	if panel.Depressed or panel:IsSelected() then
		return DrawClose(w, h, util.ColorLighten(accentColor, 40), util.ColorAlpha(titleTextColor, 150))
	end

	if panel.Hovered then
		return DrawClose(w, h, util.ColorLighten(accentColor, 20), titleTextColor)
	end

	return DrawClose(w, h, accentColor, util.ColorAlpha(titleTextColor, 150))
end

local function DrawBack(w, h, panel, backgroundColor, textColor)
	local padding_w = 10
	local padding_h = 15

	draw.Box(0, 0, w, h, backgroundColor)
	draw.FilteredShadowedTexture(padding_w, padding_h, h - 2 * padding_h, h - 2 * padding_h, materialBack, textColor.a, textColor)
	draw.ShadowedText("back", "DermaTTT2TitleSmall", h - padding_w, 0.5 * h, textColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1)
end
...
function SKIN:PaintWindowBackButton(panel, w, h)
	...
	if panel.Depressed or panel:IsSelected() then
		return DrawBack(w, h, panel, util.ColorLighten(accentColor, 40), util.ColorAlpha(titleTextColor, 150))
	end

	if panel.Hovered then
		return DrawBack(w, h, panel, util.ColorLighten(accentColor, 20), titleTextColor)
	end

	return DrawBack(w, h, panel, accentColor, util.ColorAlpha(titleTextColor, 150))
end
