...
local function RadioComplete(cmd, arg)
   ...
   for _, cmd in pairs(RADIO.Commands) do
...
local function VoiceNotifyThink(pnl)
   ...
   if LocalPlayer():IsActive() and LocalPlayer():HasTeamRole(TEAM_TRAITOR) and pnl.ply:IsActive() and pnl.ply:HasTeamRole(TEAM_TRAITOR) then return end
