...
L.reroll_no_credits = "You need no credits to reroll!" -- will be translated after issue resolving
L.reroll_button = "Перемешать"
L.reroll_help = "Use {amount} credits to get new items in your shop!" -- will be translated after issue resolving
...
L.pickup_fail = "Вы не можете подобрать это"
...
L.item_armor_desc = [[
Уменьшает урон от пуль, огня и взрывов.
Количество брони уменьшается со временем.
Может быть куплен несколько раз. При достижении определённого количества брони становится крепче.]]
...
L.decoy_desc = [[
Показывает фальшивую метку на
радарах других команд и заставляет
Сканеры ДНК показывать местоположение
вашей Приманки, если кто-то ищет по
вашему ДНК.]]
...
L.tbut_role_toggle = "[{walkkey} + {usekey}] чтобы переключить эту кнопку для {role}"
L.tbut_role_config = "Роль: {current}"
L.tbut_team_toggle = "[SHIFT + {walkkey} + {usekey}] переключить эту кнопку для команды {team}"
L.tbut_team_config = "Команда: {current}"
L.tbut_current_config = "Текущий конфиг:"
L.tbut_intended_config = "Предназначеный конфиг создателем карты:"
L.tbut_admin_mode_only = "Видно только вам, потому что вы администратор и '{cv}'' стоит на '1'"
...
L.f1_dmgindicator_title = "Настройки индикатора повреждения"
L.f1_dmgindicator_enable = "Включить"
L.f1_dmgindicator_mode = "Выбрать тему индикатора повреждения"
L.f1_dmgindicator_duration = "Длительность индикатора после попадания(секунд)"
L.f1_dmgindicator_maxdamage = "Урон, необходимый для максимальной непрозрачности"
L.f1_dmgindicator_maxalpha = "Максимальная непрозрачность индикатора повреждения"
...
-- 2020-01-04
L.doubletap_sprint_anykey = "Включить двойное нажатие, для спринта, пока не перестанете двигаться"
L.doubletap_sprint_anykey_tip = "Вы будете бежать, пока продолжаете двигаться"

L.disable_doubletap_sprint = "Отключить двойное нажатие, для спринта"
L.disable_doubletap_sprint_tip = "Двойное нажатие клавиши больше не включит бег"
