...
-- this works on clients and servers, however you can only remove decals in the
-- realm they were created
...
function util.Decal(name, startpos, endpos, filter)
	-- call the removable decal function while also creating a unique name
	return util.DecalRemovable(name .. tostring(table.Count(decals)), name, startpos, endpos, filter)
end
