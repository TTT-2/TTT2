...
if CLIENT then -- CLIENT
	...
	function HUDELEMENT:PerformLayout()
		self.scale = GLAPP.GetGlobalScale()
		self.basecolor = self:GetHUDBasecolor()
	...
	function HUDELEMENT:DrawComponent(name)
		...
		draw.AdvancedText(name, "PureSkinBar", x + self.iconSize + self.pad, y + h * 0.5, util.GetDefaultColor(self.basecolor), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, true, self.scale)
		self:DrawLines(x, y, w, h, self.basecolor.a)

		local nSize = self.iconSize - 16

		draw.FilteredShadowedTexture(x, y - 2 - (nSize - h), nSize, nSize, self.icon, 255, util.GetDefaultColor(self.basecolor), self.scale)
	end
