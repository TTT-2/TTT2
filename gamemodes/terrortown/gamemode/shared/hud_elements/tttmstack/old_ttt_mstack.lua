...
if CLIENT then
	...
	local function PrepareItem(item, bg_col)
		...
		if item.image then
			...
			item.title_wrapped, _, _ = WrapText(item.title, max_text_width, item.title_spec.font)
		...
		item.text_wrapped, _, _ = WrapText(item.text, max_text_width, item.text_spec.font)
