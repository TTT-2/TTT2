...
if CLIENT then
	...
	function HUDELEMENT:DrawBar(x, y, w, h, tipSize, alpha, item)
		...
		draw.SimpleText(item.name, font, x + tipSize + bordersize + pad + 2, y + h * 0.5 + 2, Color(0, 0, 0, alpha * 0.75), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText(item.name, font, x + tipSize + bordersize + pad, y + h * 0.5, Color(255, 255, 255, alpha), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

		if item.amount then
			draw.SimpleText(item.amount, font, x + w - pad, y + h * 0.5 + 2, Color(0, 0, 0, alpha * 0.75), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
