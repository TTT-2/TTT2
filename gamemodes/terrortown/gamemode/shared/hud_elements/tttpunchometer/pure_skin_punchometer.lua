...
if CLIENT then
	...
	function HUDELEMENT:PunchPaint()
		...
		util.DrawAdvancedText(L.punch_help, "TabLarge", x + w * 0.5, y, self:GetDefaultFontColor(self.basecolor), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, true, self.scale)
		...
		if bonus ~= 0 then
			...
			util.DrawAdvancedText(text, "TabLarge", x + w * 0.5, y + self.margin * 2 + 20, self:GetDefaultFontColor(self.basecolor), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, true, self.scale)
		end
	end
	...
	function HUDELEMENT:Draw()
		...
		if IsValid(tgt) and not tgt:IsPlayer() and tgt:GetNWEntity("spec_owner", nil) == client then
			...
			util.DrawAdvancedText(interp(L.spec_help, key_params), "TabLarge", x + self.size.w * 0.5, y, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, true, self.scale)
			if GetConVar("ttt_spectator_mode"):GetBool() then
				util.DrawAdvancedText(interp(L.spec_help2, key_params), "TabLarge", x + self.size.w * 0.5, y +20, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, true, self.scale)
			end
		end
	end
end
