...
if CLIENT then
	...
	function HUDELEMENT:Draw()
		...
		if rd then
			...
			util.DrawAdvancedText(string.upper(text), "PureSkinRole", nx, ry, self:GetDefaultFontColor(self.basecolor), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, true, Vector(role_scale_multiplier * 0.9, role_scale_multiplier, role_scale_multiplier))
		end
		...
		if calive then
			...
			if cactive and isfunction(self.secondaryRoleInformationFunc) then
				...
				if secInfoTbl and secInfoTbl.color and secInfoTbl.text then
					...
					util.DrawAdvancedText(sri_text_caps, "PureSkinBar", nx2 + sri_width * 0.5, ry, self:GetDefaultFontColor(secInfoTbl.color), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, true, self.scale)
